...
def test_for_deep_ensemble(models, datatest, args):
    ...
    for data, target in data_loader:
        ...
        avg_log_probs_view = avg_log_probs.view(-1,1)
        avg_log_probs_view = F.softmax(avg_log_probs_view, dim=0)
        one_hot_vector = torch.zeros_like(avg_log_probs_view)
        max_index = torch.argmax(avg_log_probs_view)
        one_hot_vector[max_index] = 1
        brier_scores.append(brier_score_loss(one_hot_vector.cpu().detach().numpy(), avg_log_probs_view.cpu().detach().numpy()))
